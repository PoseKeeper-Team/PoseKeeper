...
| `extract_face_landmarks(frame)` | `np.ndarray (1404,)` | LSTM 집중도 입력 (468 landmarks × xyz) |
...
| `estimate_head_pose(face_landmarks)` | `tuple[float, float, float]` | yaw, pitch, roll — 시선 이탈 감지 |
...
### 6. `estimate_head_pose(face_landmarks)`
...
| 강지윤 | LSTM 집중도 분석 | `np.ndarray (1404,)` 시퀀스 | `extract_face_landmarks()` + EAR/MAR |
...
6. `estimate_head_pose()` 구현 (solvePnP)
7. 전체 통합 — 웹캠 루프에서 실제 프레임으로 동작 확인
...
- [ ] `compute_mar()` → `float`
- [ ] `estimate_head_pose()` → `(yaw, pitch, roll)` 각도값
